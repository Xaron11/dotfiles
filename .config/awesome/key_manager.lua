...
function key_manager.get_client_keys()
...
local function get_tag_keys(tag_id)
    local tagkeys = gears.table.join(
        awful.key({ modkey }, tag_id,
            function()
        ...
        -- Toggle tag display.
        awful.key({ modkey, "Control" }, tag_id,
            function()
                local screen = awful.screen.focused()
                local tag = screen.tags[tag_id]
                if tag then
        ...
        awful.key({ modkey, "Control", "Shift" }, tag_id,
            function()
                if client.focus then
                    local tag = client.focus.screen.tags[tag_id]
                    if tag then
        ...
        awful.key({ "Mod1" }, "t", function()
            --awful.spawn.with_shell("tdrop -ma -w 70% -x 15% -y $(($PANEL_HEIGHT + 10)) alacritty")
